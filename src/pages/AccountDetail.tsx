/** Account detail: blended overview plus a tab per connected platform. */
import { useState } from 'react'
import { Link, useNavigate, useParams } from 'react-router'
import {
  Area, Bar, BarChart, CartesianGrid, ComposedChart, ResponsiveContainer, Tooltip, XAxis, YAxis,
} from 'recharts'
import { ArrowLeft, FileText, ShieldCheck, Timer, TrendingUp, TrendingDown } from 'lucide-react'
import { useApp } from '@/context/app'
import {
  getAccount, metricsFor, pacing, health, alertsFor, PLATFORMS, RANGES, type Account, type PlatformId,
} from '@/lib/data'
import { money, money2, moneyK, num, compact } from '@/lib/format'
import {
  Card, Stat, Button, Segmented, SectionTitle, HealthBadge, PacingBar, Chip, SeverityDot,
} from '@/components/ui/kit'

const TOOLTIP = {
  background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 10,
  boxShadow: 'var(--shadow-pop)', fontSize: 12, color: 'var(--ink)',
}

export default function AccountDetail() {
  const { id = '' } = useParams()
  const account = getAccount(id)
  const { range, setRange, role } = useApp()
  ...
  if (!account) {
    return (
      <div className="text-center py-16">
        <p className="text-[14px] text-[var(--ink-2)] mb-4">That account does not exist.</p>
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex flex-wrap items-center gap-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-2.5 mt-0.5">
            <span className="text-[13px] text-[var(--ink-2)]">{account.trade} · {account.location}</span>
            <HealthBadge health={health(account)} />
          </div>
        </div>
        <div className="ml-auto flex items-center gap-2.5">
          <Segmented value={range} onChange={setRange} options={RANGES.map((r) => ({ value: r.id, label: r.label }))} />
          <Button variant="primary" onClick={() => navigate(`/reports?account=${account.id}`)}><FileText size={15} /> Export report</Button>
        </div>
      </div>
      ...
      <div className="overflow-x-auto clip-scroll -mx-1 px-1">
        <Segmented
          value={tab}
          onChange={setTab}
          options={[{ value: 'overview' as const, label: 'Overview' }, ...PLATFORMS.map((p) => ({ value: p.id, label: p.short }))]}
        />
      </div>

      {tab === 'overview' && <Overview account={account} ownerView={role === 'owner'} range={range} />}
...
function Overview({ account, ownerView, range }: { account: Account; ownerView: boolean; range: any }) {
  ...
  const trend = account.leadsDaily.slice(-30).map((leads, i) => ({
    d: i, leads: Math.round(leads), spend: Math.round(account.spendDaily.slice(-30)[i]),
  }))
    ...
    <div className="flex flex-col gap-6">
              ...
              return (
                <div key={pl.id} className="bg-[var(--surface-2)] border border-[var(--line)] rounded-[9px] px-3.5 py-3 flex flex-col gap-1.5">
                  <div className="flex items-center justify-between">
                    <span className="font-semibold text-[12.5px]">{pl.short}</span>
      ...
      <section>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <TrendCard title="Leads / day" data={trend.map((r) => ({ d: r.d, v: r.leads }))} color="var(--s-blue)" />
          <TrendCard title="Spend / day" data={trend.map((r) => ({ d: r.d, v: r.spend }))} color="var(--s-orange)" fmt={(v) => money(v)} />
        </div>
      </section>

      {ownerView && (
        <section>
          <SectionTitle>Account economics <span className="text-[11px] font-medium text-[var(--muted)] ml-2">Owner seat</span></SectionTitle>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3.5">
    ...
    <ResponsiveContainer width="100%" height={130}>
      {bars ? (
        <BarChart data={rows} margin={{ top: 4, right: 4, bottom: 0, left: -24 }}>
          <CartesianGrid stroke="var(--line)" vertical={false} />
          <YAxis tick={{ fontSize: 10, fill: 'var(--muted)' }} axisLine={false} tickLine={false} width={34} />
        ...
        <ComposedChart data={rows} margin={{ top: 4, right: 4, bottom: 0, left: -24 }}>
          <defs>
            <linearGradient id={'m' + color.replace(/\W/g, '')} x1="0" y1="0" x2="0" y2="1">
          ...
          <YAxis tick={{ fontSize: 10, fill: 'var(--muted)' }} axisLine={false} tickLine={false} width={34} />
          <Tooltip contentStyle={TOOLTIP} />
          <Area type="monotone" dataKey="v" stroke={color} strokeWidth={2} fill={`url(#m${color.replace(/\W/g, '')})`} isAnimationActive={false} />
        </ComposedChart>
      )}
    </ResponsiveContainer>
  )
...
function PlatformChart({ title, data, color, bars }: { title: string; data: number[]; color?: string; bars?: boolean }) {
  return (
    <Card className="p-4">
      <div className="eyebrow mb-2">{title}</div>
      <MiniChart data={data} color={color} bars={bars} />
    </Card>
  )
}
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3.5">
        ...
        <Card className="p-4 flex flex-col gap-2">
          <span className="eyebrow">Google Guaranteed</span>
          <div className="flex items-center gap-2"><ShieldCheck size={22} style={{ color: d.guaranteed ? 'var(--st-good)' : 'var(--st-critical)' }} /><span className="font-semibold text-[16px]">{d.guaranteed ? 'Verified' : 'Pending'}</span></div>
          <span className="text-[11.5px] text-[var(--muted)]">{d.guaranteed ? 'Badge live on placements' : 'Background check in review'}</span>
        </Card>
        <Card className="p-4 flex flex-col gap-2">
          <span className="eyebrow">Avg response</span>
          <div className="flex items-end gap-1.5"><Timer size={20} style={{ color: d.responseMins < 5 ? 'var(--st-good)' : 'var(--st-serious)' }} /><span className="mono text-[24px] font-semibold leading-none">{d.responseMins.toFixed(1)}</span><span className="text-[12px] text-[var(--muted)] mb-0.5">min</span></div>
          <span className="text-[11.5px] text-[var(--muted)]">{d.responseMins < 5 ? 'Within target' : 'Above 5 min target'}</span>
        </Card>
      </div>
